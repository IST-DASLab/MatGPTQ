...
def calculate_total_bits(
        current_bitwidths: List[List[float]],
...
def load_layers(
    ...
    """
    Update model weights in-place to match the slice_bitwidths given in new_state.
    Assumes model currently uses plain nn.Linear layers (not MatryoshkaLinear).
    """
    ...
    with torch.no_grad():
        for g in range(num_groups):
            for layer_name, new_level, old_level in zip(
                grouped_layer_names[g], new_state[g], model.state[g]
            ):
                # Skip unchanged slice bitwidth
                ...
                # Load quant tensors for this layer
                layer_data = torch.load(
                    os.path.join(quant_weights_path, layer_name, "data.pt"),
...
def compute_fitness(
    model, data, fitness_fn, target_logits: Optional[torch.Tensor] = None
) -> float:
    if fitness_fn == "ppl":
...
def selection(
    ...
    for candidate in candidates:
        ...
        fitness = compute_fitness(
            model, calibration_minibatch, fitness_fn, target_logits_minibatch
        )
...
def parse_args():
    ...
    parser.add_argument("--eval_datasets", nargs="+", type=str, default=["fineweb_edu", "wikitext2", "c4"], help="Datasets used for evaluation")
    ...
    parser.add_argument("--target_bitwidth", type=float, required=True, help="Base level for all layers. If no integer, initialize random with this average")
    parser.add_argument("--quant_weights_path", type=str, required=True, help="Path to quantized weights")
    parser.add_argument("--survivors_per_selection", type=int, nargs="+", required=True, help="Number of survivors after each stage of selection")
    parser.add_argument("--tokens_per_selection", type=int, nargs="+", required=True, help="Number of calibration tokens at each stage of selection")
    parser.add_argument("--initially_generated", type=int, help="Only for non-integer initial level: Number of search points generated in the beginning; fittest are selected for the initial population")
    parser.add_argument("--initial_tokens", type=int, help="Only for non-integer initial level: Number of calibration tokens used for the initial generation")
    parser.add_argument("--group_rule", type=str, default="size", choices=["size", "name", "none"], help="Layer grouping rule. Mutations are performed only within a group.")
    parser.add_argument("--kl_topk", type=int, default=10, help="TopK logits in KL-divergence (for sparse_kl fitness function)",)
    parser.add_argument("--step_size", type=int, default=1, help="Step size between adjacent levels")
    # Misc params
    ...
    parser.add_argument("--attn_implementation", type=str, default=None, choices=["eager", "sdpa", "flash_attention_2"], help="Attention: eager, sdpa, or flash_attention_2")
...
def main():
    ...
    for layer_name in layer_names:
        available_bitwidths[layer_name] = list(zip([2, 3, 4, 6, 8], [layer_name for _ in range(6)]))
        ...
        for group_names in grouped_layer_names:
            ...
            for layer_name in group_names:
                ...
                if args.target_bitwidth in available_bws:
                    group_bitwidths.append(int(args.target_bitwidth))
                else:
                    # Find closest
                    closest = min(available_bws, key=lambda x: abs(x - args.target_bitwidth))
                    group_bitwidths.append(int(closest))
        ...
        for _ in range(args.initially_generated):
            ...
            while candidate_bits > target_bits and iterations < max_iterations:
                ...
                for i in range(len(grouped_layer_names[group_id])):
                    next_bw = get_next_bitwidth(candidate, target_bits, grouped_layer_names, available_bitwidths, model, group_id, i, 'decrease')
                ...
                new_bw = get_next_bitwidth(candidate, target_bits, grouped_layer_names, available_bitwidths, model, group_id, decr_id, 'decrease')
    ...
    for generation in range(args.generations):
        ...
        if generation % args.eval_every == 0:
            for eval_dataset_name, eval_dataset in zip(
        ...
        while len(offspring_list) < args.offspring:
            ...
            if args.group_rule == "none":  # there can be mutations between layers of different sizes
                ...
                while offspring_bits > target_bits and iterations < max_iterations:
                    ...
                    decr_ids = []
                    for i in range(len(grouped_layer_names[group_id])):
                        next_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, group_id, i, 'decrease')
                        if next_bw is not None:
                            decr_ids.append(i)
                    ...
                    new_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, group_id, decr_id, 'decrease')
                ...
                for _ in range(num_flips):
                    ...
                    incr_ids = []
                    for i in range(len(grouped_layer_names[group_id])):
                        next_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, group_id, i, 'increase')
                        if next_bw is not None:
                            incr_ids.append(i)
                    ...
                    if len(incr_ids) == 0:
                        ...
                        for _ in range(3):  # Try up to 3 times
                            decrease_attempts += 1
                            decr_group_id = random.choices(range(len(grouped_layer_names)), weights=[len(g) for g in grouped_layer_names])[0]

                            decr_ids = []
                            for i in range(len(grouped_layer_names[decr_group_id])):
                                next_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, decr_group_id, i, 'decrease')
                                if next_bw is not None:
                                    decr_ids.append(i)
                            ...
                            if decr_ids:
                                ...
                                new_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, decr_group_id, decr_id, 'decrease')
                    ...
                    if incr_ids:
                        ...
                        new_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, group_id, incr_id, 'increase')
                ...
                for _ in range(num_flips):
                    ...
                    if len(incr_ids) == 0:
                        ...
                        if other_decr_id is not None:
                            new_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, group_id, other_decr_id, 'decrease')
                    ...
                    # Perform the swap
                    new_decr_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, group_id, decr_id, 'decrease')
                    offspring[group_id][decr_id] = new_decr_bw

                    new_incr_bw = get_next_bitwidth(offspring, target_bits, grouped_layer_names, available_bitwidths, model, group_id, incr_id, 'increase')
        ...
        for i in range(num_groups):
            f.write(
                "\n".join(
                    [
                        f"{layer_name}: {level}"
                        for layer_name, level in zip(grouped_layer_names[i], parent[i])
                    ]
                )
            )
            if i != num_groups - 1:
