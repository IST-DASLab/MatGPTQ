...
from src.quant_utils_gptq import Quantizer
...
class GPTQ:
    ...
    def __init__(
        ...
        self.quantizer.configure(bits=self.bitwidth, perchannel=perchannel, sym=sym)
            ...
            for c1 in range(0, d_col, block_size):
                    ...
                    for i in range(ncols):
                        ...
                        if self.group_size and (c1 + i) % group_size == 0:
                            self.quantizer.find_params(w[:, (c1 + i) : (c1 + i + group_size)], weight=True)
