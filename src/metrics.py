...
@torch.no_grad()
def compute_perplexity_(model, data, batch_size):
    """
    model: AutoModelForCausalLM (Gemma 2)
    data:  list of [1, seqlen] LongTensors from get_wikitext2(...)
    """
    ...
    nlls = []
    seqlen = data[0].size(1)

    for batch in data:
        # batch: [1, seqlen]
        batch = batch.to(device)

        # match the 'elif "llama" in args.net ...: outputs = lm.model.model(batch)' path:
        # for HF Gemma2ForCausalLM, calling model(...) returns (logits, ...)
        outputs = model(batch)
        logits = outputs.logits                 # [1, seqlen, V]

        shift_logits = logits[:, :-1, :]        # [1, seqlen-1, V]
        shift_labels = batch[:, 1:]             # [1, seqlen-1]
        ...
        # original code multiplies by lm.seqlen to turn mean loss into total NLL
        neg_log_likelihood = loss.float() * seqlen
        nlls.append(neg_log_likelihood)

    nsamples = len(data)
    # same formula as original: exp(sum(NLL) / (nsamples * seqlen))
...
def compute_perplexity_layer_per_layer(
    ...
    # layers[0].attention_type = "sliding_attention"
