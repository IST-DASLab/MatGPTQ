...
from src import linalg_utils as linalg_utils
from src.quant_utils_matgpqt import MatryoshkaQuantizer
...
class MatGPTQ:
    ...
    def __init__(
        ...
        assert max(bitwidth_options) <= 8, "Master bitwidth cannot exceed 8 bits."
        self._validate_layer(layer)
        self.layer = layer
        self.W = self.layer.weight
        self.d_row, self.d_col = model_utils.get_number_of_rows_and_cols(layer)
        # MatGPTQ hyperparameters
        self.master_bitwidth = max(bitwidth_options)
        ...
        self.quantizer.configure(bits=self.master_bitwidth, perchannel=perchannel, sym=sym)
    ...
    @torch.no_grad()
    def step(self) -> Tuple[dict, dict, dict, Optional[Tensor]]:
        # 1) define constants and chunk
        d_row, d_col, block_size, device, dtype = (
            self.d_row,
            self.d_col,
            self.block_size,
            self.W_device,
                ...
                for i in range(ncols):
                    ...
                    if self.group_size and (c1 + i) % group_size == 0:
                        quantizer.find_params(
                            w[:, (c1 + i) : (c1 + i + group_size)], weight=True
                        )
                        scale[:, (c1 + i) // group_size] = quantizer.scale.flatten()
                        zero[:, (c1 + i) // group_size] = quantizer.zero.flatten()

                    q, loss = quantizer.quantize(
                        w_ci.unsqueeze(1), self.bitwidth_options, self.bitwidth_weights
                    )
                    err = loss.flatten()
                    qweight[:, c1 + i] = q.flatten()
                    err = (err / len(self.bitwidth_options)) / d
                    losses_blk[:, i] = err**2

                    w[:, c1 + i] = quantizer.dequantize(q, self.master_bitwidth, self.master_bitwidth).flatten()
